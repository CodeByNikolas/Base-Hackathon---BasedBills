  ...
  try {
    ...
    console.log('Environment variables in API route:', {
      walletConnectProjectId: process.env.NEXT_PUBLIC_CDP_PROJECT_ID
        ? 'Set'
        : 'Not set',
      onchainKitApiKey: process.env.NEXT_PUBLIC_ONCHAINKIT_API_KEY ? 'Set' : 'Not set',
    ...
    const missingVars = [];
    if (!walletConnectProjectId) missingVars.push('NEXT_PUBLIC_CDP_PROJECT_ID');
    if (!onchainKitApiKey) missingVars.push('NEXT_PUBLIC_ONCHAINKIT_API_KEY');
    if (!cdpProjectId) missingVars.push('NEXT_PUBLIC_CDP_PROJECT_ID');
